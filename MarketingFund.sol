...
contract MarketingFund {
    ...
    mapping(address => bool) public isInFund;
    ...
    event MemberDeleted(address _toDelete);
    ...
    modifier fundMember {
        require(isInFund[msg.sender], "You're not able to interact with this Smart Contract");
        _;
    }
    ...
    function addMember(address _newMember) public onlyOwner {
        isInFund[_newMember] = true;
        emit MemberAdded(_newMember);
    }

    function deleteMember(address _toDelete) public onlyOwner {
        isInFund[_toDelete] = false;
        emit MemberDeleted(_toDelete);
    }
