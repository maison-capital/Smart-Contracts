...
contract PrivatePresale {
    ...
    function changeRefundStatus() public onlyOwner {
        if (!refundProcess) {
            refundProcess = true;
        } else {
            refundProcess = false;
    ...
    function addAddresses(address[] memory _addresses, uint256[] memory _ammounts) public onlyOwner {
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            inPresale[_addresses[i]].newAmount = (_tokenAmount*10**2)/privatePrice;
            inPresale[_addresses[i]].tgeCollect = (((_tokenAmount*10**2)/privatePrice)*tgeToCollect)/(1*10**5);
            inPresale[_addresses[i]].nrmCollect = (((_tokenAmount*10**2)/privatePrice)*normalCollect)/(1*10**5);
        }
    }
